...
class TokenManager {
  ...
  Future<String?> refreshToken() async {
    ...
    try {
      // Get user info
      final userInfoJson = await _secureStorage.read(StorageKeys.userCredentials);
  ...
  Future<Map<String, String>> _callRefreshTokenApi(String refreshToken) async {
    ...
    final accessTokenExpiry = DateTime.now().add(Duration(minutes: _accessTokenExpiryMinutes));
    ...
    final refreshTokenExpiry = DateTime.now().add(Duration(days: _refreshTokenExpiryDays));
  ...
  void _incrementRefreshAttempt(String username) {
    ...
    if (_refreshAttempts[username]! >= _maxRefreshAttempts) {
      ...
      AppLogger.warning('Rate limit exceeded for user: $username. Locked out for $_refreshLockoutDuration');
    }
  }
