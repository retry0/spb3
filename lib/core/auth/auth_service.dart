...
class AuthService {
  ...
  Future<void> _handleOfflineToOnlineTransition() async {
    ...
    try {
      ...
      if (!isValid) {
        ...
        AppLogger.warning(
          'Token invalid after reconnection, requiring re-authentication',
        );
  ...
  Future<bool> login(String username, String password) async {
    try {
      ...
      if (response.statusCode == 200 && response.data != null) {
        ...
        final refreshToken =
            responseData['refreshToken'] ?? _createRefreshToken(accessToken);
  ...
  Future<bool> _offlineLogin(String username, String password) async {
    try {
      ...
      final storedCredentialsJson = await _secureStorage.read(
      ...
      final storedCredentials =
          jsonDecode(storedCredentialsJson) as Map<String, dynamic>;
  ...
  String _createRefreshToken(String accessToken) {
    try {
      ...
      final payload = base64Encode(
        utf8.encode(jsonEncode(refreshTokenPayload)),
      );
  ...
  String _createOfflineRefreshToken(String username, String userId) {
    ...
    final expiryTime = now.add(
      Duration(days: 90),
    ); // Offline refresh tokens last longer
