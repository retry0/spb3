...
/// Service responsible for managing Kendala forms using SQLite storage
class KendalaFormSqliteService {
  final Dio _dio;
  ...
  /// Save a Kendala form to SQLite database
  Future<bool> saveForm({
    ...
    try {
      ...
      if (existingRecords.isEmpty) {
        // Insert new record
        await db.insert('kendala_forms', dbData);
      } else {
      ...
      AppLogger.info('Kendala form saved to SQLite for SPB: $spbId');
      return true;
    } catch (e) {
      AppLogger.error('Error saving Kendala form data to SQLite: $e');
      return false;
    }
  }
  ...
  Future<Map<String, dynamic>?> getFormData(String spbId) async {
    try {
      ...
      return {
        ...
        'isAnyHandlingEx': dbData['is_any_handling_ex'] as String,
        'timestamp': dbData['timestamp'] as int,
      };
    } catch (e) {
      AppLogger.error('Error retrieving form data from SQLite: $e');
      return null;
    }
  }
  ...
  Future<void> clearAllSyncData() async {
    try {
      final db = await _dbHelper.database;
      await db.delete('kendala_forms');
      AppLogger.info('Cleared all Kendala form data from SQLite');
    } catch (e) {
      AppLogger.error('Failed to clear Kendala form data: $e');
      throw Exception('Failed to clear Kendala form data: $e');
    }
  }
