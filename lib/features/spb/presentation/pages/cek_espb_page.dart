...
class _CekEspbPageState extends State<CekEspbPage>
    ...
    try {
      ...
      if (data.containsKey('status')) {
        if (data['status'] is bool) {
          ...
          if (value != "0" && value != "1") {
            data['status'] = value == "true" || value == "yes" || value == "True" || int.tryParse(value) == 1
                ? "1"
                : "0";
          }
        }
  ...
  Future<void> _saveToDatabase(String spbId, Map<String, dynamic> data) async {
    try {
      ...
      if (existingRecords.isEmpty) {
        // Insert new record
        await _dbHelper.insert('espb_form_data', {
          'no_spb': spbId,
          ...
          'alasan': data['alasan'] ?? '',
          'is_any_handling_ex': "False", // Default for cek form
  ...
  Future<bool> _syncWithApi(Map<String, dynamic> data, {int retryCount = 0}) async {
    try {
      ...
      print('DioException syncing form (attempt ${retryCount + 1}): ${e.message}');
