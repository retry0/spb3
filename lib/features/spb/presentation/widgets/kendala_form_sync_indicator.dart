...
class KendalaFormSyncIndicator extends StatefulWidget {
  ...
  const KendalaFormSyncIndicator({
    super.key,
    this.spbNumber,
    this.compact = false,
    this.onRetry,
  ...
  @override
  State<KendalaFormSyncIndicator> createState() => _KendalaFormSyncIndicatorState();
}

class _KendalaFormSyncIndicatorState extends State<KendalaFormSyncIndicator> {
  ...
  Future<void> _checkSyncStatus() async {
    ...
    if (widget.spbNumber != null) {
      ...
      final isSynced = await _syncManager.isFormSynced(widget.spbNumber!);
  ...
  void _handleRetry() {
    if (widget.onRetry != null) {
      ...
      if (widget.spbNumber != null) {
        _syncManager.syncForm(widget.spbNumber!).then((_) {
          _checkSyncStatus();
        });
      } else {
        _syncManager.forceSyncNow().then((_) {
          _checkSyncStatus();
        });
      }
    }
    ...
    return ValueListenableBuilder<SyncStatus>(
      ...
      builder: (context, syncStatus, _) {
        return ValueListenableBuilder<String?>(
          valueListenable: _syncManager.errorMessageNotifier,
          builder: (context, errorMessage, _) {
            return ValueListenableBuilder<DateTime?>(
              valueListenable: _syncManager.lastSyncTimeNotifier,
